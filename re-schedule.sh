...
kubens ito5 && kubectl patch deployments wordpress -p '{"spec": {"template": {"spec": {"nodeSelector": {"kubernetes.io/hostname": "sail-2"}}}}}'
kubens ito6 && kubectl patch deployments wordpress -p '{"spec": {"template": {"spec": {"nodeSelector": {"kubernetes.io/hostname": "sail-5"}}}}}'
